...
def run(app_state, progress_callback=None, log_callback=None):
    ...
    data = app_state.data or {}
    characters_data = data.get("characters", {})
    characters = characters_data.get("characters", [])

    if not characters:
        raise ValueError("data/characters.json의 characters가 비어 있습니다.")
    ...
    for index, character in enumerate(characters, start=1):
        ...
        if saved_file_count >= expected_docs:
            log(f"[collect] 건너뜀 {character} ({index}/{total_characters}, 파일 {saved_file_count}개)")
    ...
    # ✅ 1단계 끝나면 바로 teams.json / gear.json 생성
    try:
        from postprocess.run_post import run as run_post

        run_post(app_state, log_callback=log)
        log("[collect] teams.json / gear.json 자동 생성 완료")
    except Exception as e:
        log(f"[collect][postprocess 오류] {e}")
